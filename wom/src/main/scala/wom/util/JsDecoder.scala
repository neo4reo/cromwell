...
class JsDecoder {
  ...
  /**
    * Returns the first type in the collection, or WomNothingType
    */
  def womValuesType(womValues: Iterable[WomValue]): WomType = {
    womValues.headOption.map(_.womType).getOrElse(WomNothingType)
  }
  ...
  def decodeMap(map: Map[String, AnyRef]): ErrorOr[WomValue] = {
    map.traverse({
      ...
      mapped =>
        WomMap(WomMapType(WomStringType, womValuesType(mapped.values)), mapped)
    })
  }
}
